...
from .conftest import PASSWORD, login_headers, make_portal_user, make_user
...
async def test_create_user_validation(client, admin, junction):
    ...
    await make_portal_user(client, admin, "hospital.boss@example.com", "HOSPITAL")
...
async def test_list_users_and_role_filter(client, admin):
    ...
    await make_portal_user(client, admin, "u2@example.com", "HOSPITAL")
    r = await client.get(
...
async def test_disabling_a_user_kills_login_and_token(client, admin):
    u = await make_portal_user(client, admin, "temp@example.com", "HOSPITAL")
...
async def test_password_reset_returns_new_password_once(client, admin):
    u = await make_portal_user(client, admin, "reset.me@example.com", "HOSPITAL")
    r = await client.post(
